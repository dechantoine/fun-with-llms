...
from .llm_providers.gemma_models import LocalGemma
...
@logger.catch
def init_models() -> dict[str, LocalGemma]:
    ...
    for model in models:
        ...
        dict_models[model_name] = LocalGemma(model_name=model)
    return dict_models

@logger.catch
async def generate(model: LocalGemma,
                   model_name: str,
                   input_row: dict,
                   max_retries: int = 0,
...
@logger.catch
def prepare_jobs(dict_models: dict[str, LocalGemma]) -> ListInterleavedJobs:
